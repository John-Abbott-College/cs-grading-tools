...
def run_manual_tests(autograding_answers):
    ...
    section_filename.ask_question_with_partial_grade("read_airport_temperatures uses parameter, not hardcode to file path", 1)

    section_well_tested = grading.add_section("if __name__ == '__main__' used correctly", 1)
    section_well_tested.ask_question_with_partial_grade("temperatures.py produces plot and no extra output when temperatures.py run", 1)

    section_plot_data = grading.add_section("Plot data", 1)
    section_plot_data.ask_question_with_partial_grade("Plot is a bar chart with extremes above and below y-axis", 1)

    section_plot_quality = grading.add_section("Plot quality", 1)
    section_plot_quality.ask_question_with_partial_grade("Plot is labelled, colored, and scaled according to instructions", 1)

    section_line_fit = grading.add_section("Linear fit", 1)
    section_line_fit.ask_question_with_partial_grade("Linear fit is computed for extreme hot and cold days", 1)

    section_line_fit_plot = grading.add_section("Linear fit plot", 1)
    section_line_fit_plot.ask_question_with_partial_grade("Linear fit is included in plot", 1)

    section_line_fit_print = grading.add_section("Linear fit print", 1)
    section_line_fit_print.ask_question_with_partial_grade("Rate of change is printed", 1)

    section_edits_required = grading.add_section("Submission issues", 1)
    section_edits_required.ask_question_with_partial_grade("no other major edits required to get the code to work properly", 1)
