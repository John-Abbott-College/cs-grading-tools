...
# importing files
# ============================================================================
def import_plus(file: str, inputs=None, modification_func=lambda x: x) \
...
def import_file_with_dummy_dummy_inputs(f: str) -> tuple[ModuleType, STDOUT, STDERR, str]:
...
# ============================================================================
def run_function_with_io(m, f, inputs: list[str], func_inputs: Optional[list] = None) -> list:
    """
...
# ============================================================================
def check_match_exact(expected: list[str], actual: list[str], skip_empty_lines: bool = False) -> str:
...
def errors_and_warnings(linter_infos: list[dict[str:str]]) -> tuple[list[str], list[str]]:
    errors = []
    warnings = []
...
def _linter_caught_style_mistakes(message_id, linter_info):
    infos = [f"{i['path']}: {i['line']}: {i['message']}"
             for i in linter_info if i['message-id'] == message_id]
...
style_defining_new_property_outside_init = partial(_linter_caught_style_mistakes, "W0201")
