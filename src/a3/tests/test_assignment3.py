...
from test_helper import get_inner_func, uses_loop, get_signature, get_func_calls, contains_func, uses_condition

files_to_test = {'func_file':"my_functions.py",
                 'main_file': "assignment3.py"}
...
class TestFuncSig(unittest.TestCase):
    def setUp(self):
        try:
            ...
            self.functions_to_check = ["net_income", "concentration_percent", "intake_frequency", "gcd", "hex_area","green_area"]
    ...
    def test_check_for_inner_functions(self):
        ...
        for func, inner_list in function_dict.items():
            ...
            self.assertEqual(len(inner_list), 0,
    ...
    def test_function_definitions(self):
        ...
        for func_name in self.functions_to_check:
            self.assertTrue(contains_func(self.student_code, func_name), f"This function {func_name} was not found "
    ...
    def test_function_signatures(self):
        ...
        try:
            for func_name, return_type, params_count in zip(self.functions_to_check,
                ...
                if return_type: #check only if the function returns
                    self.assertEqual(actual_return, return_type,
                                     f"Oops, the function {func_name} should return {return_type} not {actual_return}")
                self.assertEqual(params_count,actual_param_count,
        ...
        print("Good job! All functions seem to have the correct return types and number of parameters")
...
class TestQ1Evaluator(unittest.TestCase):
    ...
    def test_accuracy(self):
        ...
        gross_income = self.income_brackets[0]-10 #insuring it's a value in the first bracket
        ...
        try:
            actual_net_income = self.student_code.net_income(hourly_rate=hourly_rate,
                                                             hours_per_week=hours_per_week)

            self.assertAlmostEqual(expected_net_income,
                                   actual_net_income,
    ...
    def test_conditionals(self):
        ...
        try:
            ...
            for gross_income, tax_rate in zip(self.income_brackets, self.tax_rates):
                ...
                calculated_income = self.student_code.net_income(hourly_rate=gross_income/total_hours,
                                                                 hours_per_week=weekly_hours)

                self.assertAlmostEqual(expected_income, calculated_income, places=6, msg=
                f"""For a gross salary of {gross_income:.2f}$, I expect a net salary of {expected_income:.2f}$, 
...
class TestQ2Evaluator(unittest.TestCase):
    ...
    def test_concentration_accuracy(self):
        ...
        try:
            ...
            is_equal = (math.isclose(expected,value,abs_tol=tol) or
                        math.isclose(100*expected,value,abs_tol=tol))
            self.assertTrue(is_equal, f"concentration not calculated accurately")
        except AttributeError as attribute_err:
            print(attribute_err)
            self.fail(f"It seems like {self.file_to_test} doesn't contain the function concentration_percent()")
        except TypeError as type_err:
            print(type_err)
            self.fail(
                f"It seems like the function concentration_percent isn't defined with the right number of input parameters")

        self.assertIsNotNone(value, f"It seems like the function concentration_percent() isn't returning a value")
        self.assertIsInstance(value, float, f"It seems like the function concentration_percent() "
    ...
    def test_logic(self):
        ...
        try:
            ...
            self.assertTrue(uses_condition(function_to_test),
            ...
            self.assertTrue(uses_loop(function_to_test),
                            f"Oops, it seems like the active substance intake frequency is missing an important "
            ...
            self.assertTrue(is_calling_helper, f"The function {function_to_test.__name__}() is not calling {helper_func_name}()")

        except AttributeError as a:
            self.fail(a)
        except TypeError as e:
            self.fail(e)
        print("Great! You used all the statements and to determine the intake frequency")
    @weight(4)
    def test_call_intake_frequency(self):
        ...
        try:
            ...
            self.assertAlmostEqual(first=actual, second=expected,
    ...
    def test_intake_frequency_upper_bound(self):
        ...
        try:
            actual = self.student_code.intake_frequency(half_life=half_life)
            self.assertAlmostEqual(first=actual,second=expected, msg =f"Tested intake_frequency() with the caffeine's {half_life=} in hours."
                                                                      f" Got: {actual} but expected {round(expected)}", delta=1)
        except AttributeError as a:
            self.fail(a)
        except TypeError as e:
            self.fail(e)
        print("Perfect! intake_frequency() takes into account the upper limit correctly")
...
class TestQ3Evaluator(unittest.TestCase):
    ...
    def test_gcd_logic(self):
        ...
        try:
            ...
            self.assertTrue(uses_condition(function), f"gcd() isn't using a conditional statement")
        except Exception as e:
            self.fail(e)
    @weight(4)
    def test_gcd_accuracy(self):
        ...
        try:
            for pair, expected in zip(inputs,expected_output):
                ...
                self.assertEqual(expected, actual, f"Incorrect gcd for {pair}. Expected: {expected} Got: {actual}")
        except Exception as e:
            self.fail(e)
...
class TestQ4Evaluator(unittest.TestCase):
    ...
    def test_hex_area(self):
        ...
        try:
            ...
            self.assertAlmostEqual(expected_area,actual_area,FLOAT_PRECISION_TOL,
                                   f"Hmm... the area of a hex of a={a}, should be {expected_area:.4f}, got {actual_area:.2f}")
        except AttributeError as attribute_err:
            print(attribute_err)
            self.fail(f"It seems like {files_to_test[0]} doesn't contain the tested function ")
    ...
    def test_using_loop(self):
        ...
        try:
            ...
            self.assertTrue(uses_loop(function), f"The green area cannot be obtained without using a loop")
        except Exception as e:
            self.fail(e)
    @weight(11)
    def test_green_area_accuracy(self):
        ...
        try:
            ...
            green_sides = [a0+w*i for i in range(first_green_idx,num_hex,space_between_green)]
            areas_green = [self.hex_area(a_green) - self.hex_area(a_green-w) for a_green in green_sides]
            expected_total_area  = sum(areas_green)
            actual_area = self.student_code.green_area(num_hex, a0,w)
            self.assertAlmostEqual(expected_total_area, actual_area, FLOAT_PRECISION_TOL,
                                   f"Hmm... the green area for {num_hex = }, {a0 = }, {w= } should be {expected_total_area} not {actual_area}")
        except AttributeError as attribute_err:
            print(attribute_err)
            self.fail(f"It seems like {self.file_to_test} doesn't contain the tested function ")
