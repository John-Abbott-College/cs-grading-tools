...
grading = Grading(manual_input=False)
...
def run_manual_tests(autograding_answers):
    ...
    section_plot_data.ask_question_with_partial_grade("Plot is a bar chart with extremes above and below y-axis", 1, ans=5, response_ans="(note) full marks assumed, this grade may change when reviewed by instructor.")

    section_plot_quality = grading.add_section("Plot quality", 1)
    section_plot_quality.ask_question_with_partial_grade("Plot is labelled, colored, and scaled according to instructions", 1, ans=5, response_ans="(note) full marks assumed, this grade may change when reviewed by instructor.")

    section_line_fit = grading.add_section("Linear fit", 1)
    section_line_fit.ask_question_with_partial_grade("Linear fit is computed for extreme hot and cold days", 1, ans=5, response_ans="(note) full marks assumed, this grade may change when reviewed by instructor.")

    section_line_fit_plot = grading.add_section("Linear fit plot", 1)
    section_line_fit_plot.ask_question_with_partial_grade("Linear fit is included in plot", 1, ans=5, response_ans="(note) full marks assumed, this grade may change when reviewed by instructor.")
    ...
    for test in autograder_results["tests"]:
        ...
        test_section.add_result_with_partial_grade(
            test.get("name"), test["score"], test["max_score"], test.get("output",""), response_ans="(note) for grade of 0, part marks may be awarded. Will be reviewed by instructor soon."
        )
